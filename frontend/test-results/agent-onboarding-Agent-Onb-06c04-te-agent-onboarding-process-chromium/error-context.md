...
- Name: agent-onboarding.spec.ts >> Agent Onboarding >> should complete agent onboarding process
- Location: tests/agent-onboarding.spec.ts:137:7
...
Locator: getByRole('button', { name: /Approve USDC/i })
Expected: visible
Timeout: 10000ms
...
Call log:
  ...
  - waiting for getByRole('button', { name: /Approve USDC/i })
...
- main:
  ...
  - text: ⚠️ Insufficient USDC Balance Initial Funding (USDC)
  - spinbutton: "100"
  - text: Max Auto-Price
  - spinbutton: "100"
  - text: Daily Budget
  ...
  - spinbutton: "50"
  - button "Cancel"
  - button "Insufficient Funds" [disabled]
...
```ts
  64  |           }
  65  |         },
  66  |         emit: (event: string, ...args: any[]) => {
  67  |           if (listeners[event]) {
  68  |             listeners[event].forEach(cb => cb(...args));
  69  |           }
  70  |         }
  71  |       };
  72  |       (window as any).ethereum = provider;
  73  |     }, { address: mockAddress, chainId: mockChainId });
  74  | 
  75  |     // Mock RPC calls for both localhost and 127.0.0.1
  76  |     const mockRpc = async (route: any) => {
  77  |       const request = route.request();
  78  |       if (request.method() !== "POST") return route.continue();
  79  | 
  80  |       let body;
  81  |       try {
  82  |         body = request.postDataJSON();
  83  |       } catch (e) {
  84  |         return route.continue();
  85  |       }
  86  |       
  87  |       if (!body || !body.method) return route.continue();
  88  |       const { method, params, id } = body;
  89  | 
  90  |       const respond = (result: any) => route.fulfill({
  91  |         status: 200,
  92  |         contentType: "application/json",
  93  |         body: JSON.stringify({ jsonrpc: "2.0", id, result }),
  94  |       });
  95  | 
  96  |       if (method === "eth_accounts") return respond([mockAddress]);
